package com.iexchange.spot.canal;
...
/**
 * Canal 同步现货订单到 ES（演示版）。
 */
@Slf4j
@Component
public class SpotOrderCanalSync {

    private static final DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

    private final SpotCanalProperties properties;
    private final SpotOrderEsMapper esMapper;
    private volatile boolean running;
    private CanalConnector connector;
    private Thread worker;

    public SpotOrderCanalSync(SpotCanalProperties properties, SpotOrderEsMapper esMapper) {
    ...
    public void start() {
        ...
        running = true;
        worker = new Thread(this::runLoop, "canal-spot-order-sync");
    ...
    private void handleEntries(List<CanalEntry.Entry> entries) throws Exception {
        for (CanalEntry.Entry entry : entries) {
            ...
            if (!"spot_order".equalsIgnoreCase(table)) {
                continue;
            }
            ...
            for (CanalEntry.RowData rowData : rowChange.getRowDatasList()) {
                ...
                SpotOrderDocument document = toDocument(rowData.getAfterColumnsList());
    ...
    private SpotOrderDocument toDocument(List<CanalEntry.Column> columns) {
        SpotOrderDocument document = new SpotOrderDocument();
        ...
        document.setFilledQuantity(toDecimal(getColumnValue(columns, "filled_quantity")));
